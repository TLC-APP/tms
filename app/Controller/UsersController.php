...
class UsersController extends AppController {
    ...
    public function delete($id = null) {
        ...
        if ($registeringCourse > 0) {
            $this->Session->setFlash('Người dùng là tập huấn viên của ' . $registeringCourse . ' khóa học đang đăng ký.', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-warning'));
            $this->redirect($this->referer());
        }

        $uncompletedCourse = $this->User->field('uncompletedCourse');
        if ($uncompletedCourse > 0) {
            $this->Session->setFlash('Người dùng là tập huấn viên của ' . $uncompletedCourse . ' khóa học chưa hoàn thành.', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-warning'));
            $this->redirect($this->referer());
        }
        ...
        if ($completedCourse > 0) {
            $this->Session->setFlash('Người dùng là tập huấn viên của ' . $completedCourse . ' khóa học hoàn thành.', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-warning'));
            $this->redirect($this->referer());
        }
        if ($this->User->isAdmin($id)&&$id!=Configure::read('MASTER_USERNAME')) {
            $this->Session->setFlash('Người dùng là ADMIN của hệ thống.', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-warning'));
            $this->redirect($this->referer());
        }
    ...
    public function admin_add() {
        ...
        $hocVis = $this->User->HocVi->find('list');
        $departments = $this->User->Department->find('list');
        $groups = $this->User->Group->find('list', array('conditions' => array('NOT' => array('Group.alias' => array('boss', 'guest')))));
