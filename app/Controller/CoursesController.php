    ...
    public function delete($id = null) {
        ...
        if ($this->Course->delete()) {
           $this->Session->setFlash('Xóa thành công!', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-success'));
            ...
            $this->Session->setFlash('Xóa thất bại!', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-warning'));
        }
        return $this->redirect($this->request->referer());
    }
...
// a view
    ...
    public function edit($id = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            if ($this->Course->save($this->request->data)) {
                $this->Session->setFlash('Cập nhật thất bại!', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash('Cập nhật thành công!', 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-warning'));
            }
        } else {
...
//$this->layout = 'group_select';
    ...
    public function fields_manager_index($status = null) {
        ...
        $fiels_id=$this->Course->Chapter->Field->find('all',array('recursive'=>-1,'conditions'=>array('Field.manage_user_id'=>$this->Auth->user('id'))));
        if (!empty($fiels_id)) {
            $manage_fields_id_array = Set::classicExtract($fiels_id, '{n}.Field.id');
        }
        $chapters_id=$this->Course->Chapter->getChapterByField_id($manage_fields_id_array);
        $courses_id=$this->Course->getCoursesByChapter_id($chapters_id);
        $conditions = array('Course.id' => $courses_id);
        if ($status) {
