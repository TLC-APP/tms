...
public function admin_delete($id = null) {
    ...
    public function manager_recieve($student_course_id = null) {
        ...
        if ($this->Attend->saveField('is_recieved', 1)) {
            ...
            $ngaynhan=new DateTime($this->Attend->field('recieve_date'));
            $this->Session->setFlash('Học viên đã nhận bằng thành công vào lúc: ' .$ngaynhan->format('H:i').', ngày: '.$ngaynhan->format('d/m/Y'), 'alert', array('plugin' => 'BoostCake', 'class' => 'alert-success'));
            $this->redirect($this->request->referer());
        }
    }
